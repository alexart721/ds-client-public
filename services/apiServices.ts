...
export const setUserPassword = (token: string, user: UserAuth): Promise<Response> => fetch(`${BASE_AUTH_URL}/register`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    Authorization: `Bearer: ${token}`,
...
export const loginUser = (user: UserLogin): Promise<Response> => fetch(`${BASE_AUTH_URL}/login`, {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
